...
// GetWatchlist retrieves user's watchlist movies with cursor pagination and genre filter
func (r *Repository) GetWatchlist(ctx context.Context, userID int64, cursor string, limit int, genre string) ([]WatchlistMovie, int, string, error) {
	...
	movies, totalCount, nextCursor, err := r.sharedRepo.GetUserMovies(ctx, userID, filter, movie_collection.StatusWatchlist)
	if err != nil {
		return nil, 0, "", err
	}

	return convertToWatchlistMovies(movies), totalCount, nextCursor, nil
}

// GetWatchlistIDs retrieves all TMDB IDs for user's watchlist
func (r *Repository) GetWatchlistIDs(ctx context.Context, userID int64) ([]int, error) {
	return r.sharedRepo.GetTMDBIDsByStatus(ctx, userID, movie_collection.StatusWatchlist)
}

// SearchWatchlist searches within user's watchlist
func (r *Repository) SearchWatchlist(ctx context.Context, userID int64, query string, cursor string, limit int) ([]WatchlistMovie, int, string, error) {
	filter := movie_collection.CollectionFilter{
		Query:  query,
		Cursor: cursor,
		Limit:  limit,
	...
	movies, totalCount, nextCursor, err := r.sharedRepo.SearchUserMovies(ctx, userID, filter, movie_collection.StatusWatchlist)
	if err != nil {
		return nil, 0, "", err
	}
...
func (r *Repository) AddToUserMovies(ctx context.Context, userID int64, movieDBID int, tmdbID int) error {
	return r.sharedRepo.AddToUserMovies(ctx, userID, movieDBID, tmdbID, movie_collection.StatusWatchlist)
}

// DeleteWatchlistByID removes a movie from watchlist
func (r *Repository) DeleteWatchlistByID(ctx context.Context, userID int64, id int64) error {
	return r.sharedRepo.DeleteUserMovie(ctx, userID, id, movie_collection.StatusWatchlist)
}
...
func (r *Repository) AddToUserMoviesBatch(ctx context.Context, userID int64, entries []movie_collection.UserMovieEntry) error {
	return r.sharedRepo.AddToUserMoviesBatch(ctx, userID, entries, movie_collection.StatusWatchlist)
}

// convertToWatchlistMovies converts shared UserMovie slice to WatchlistMovie slice
func convertToWatchlistMovies(movies []movie_collection.UserMovie) []WatchlistMovie {
	result := make([]WatchlistMovie, 0, len(movies))
	for _, m := range movies {
		result = append(result, WatchlistMovie{
			ID:          m.ID,
