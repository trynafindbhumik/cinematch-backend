...
// GetCurrentSuggestionForWeek gets the latest suggestion_index for user/week
func (r *Repository) GetCurrentSuggestionForWeek(ctx context.Context, userID int64, weekStart string) (int, error) {
	var maxIndex int
	...
	if err != nil {
		return 0, fmt.Errorf("failed to get current suggestion: %w", err)
	}
	return maxIndex, nil
}

// CreateSuggestion creates a new suggestion entry with given index
func (r *Repository) CreateSuggestion(ctx context.Context, userID int64, weekStart string, suggestionIndex int) (int64, error) {
	var id int64
	err := db.Pool().QueryRow(ctx, `
		INSERT INTO user_suggestions (user_id, week_start, suggestion_index)
		VALUES ($1, $2, $3)
		RETURNING id
	`, userID, weekStart, suggestionIndex).Scan(&id)
	if err != nil {
		return 0, fmt.Errorf("failed to create suggestion: %w", err)
	}
	return id, nil
}

// CreateSuggestionMovie adds a movie to a suggestion
func (r *Repository) CreateSuggestionMovie(ctx context.Context, suggestionID int64, movieID int) error {
	_, err := db.Pool().Exec(ctx, `
		INSERT INTO user_suggestion_movies (suggestion_id, movie_id)
		VALUES ($1, $2)
	`, suggestionID, movieID)
	if err != nil {
		return fmt.Errorf("failed to create suggestion movie: %w", err)
	}
	return nil
}

// GetSuggestionsByID retrieves all movies for a suggestion
func (r *Repository) GetSuggestionsByID(ctx context.Context, suggestionID int64) ([]Movie, error) {
	rows, err := db.Pool().Query(ctx, `
		SELECT m.tmdb_id, m.title, m.poster_url, m.backdrop_url, m.genres, m.release_year, m.tmdb_rating
		FROM user_suggestion_movies usm
		JOIN movies m ON usm.movie_id = m.id
		WHERE usm.suggestion_id = $1
	`, suggestionID)
	if err != nil {
